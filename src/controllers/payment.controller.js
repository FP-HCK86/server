...
exports.createPayment = async (req, res) => {
  try {
    ...
    const body = req.body || {};
    console.info('createPayment called', { userId: req.user && req.user.id, body });
    ...
    await PaymentTransaction.create({ orderId, user: user._id, tier: resolvedTier, amount, status: 'pending' });
    ...
    console.error('createPayment error', error && error.message, error && error.stack);
...
exports.paymentNotification = async (req, res) => {
  try {
    ...
    const receivedSig = body.signature_key || body.signature || body.signatureKey || '';
    ...
    const status_code = body.status_code !== undefined && body.status_code !== null ? String(body.status_code) : '';
    const gross_amount = body.gross_amount !== undefined && body.gross_amount !== null ? String(body.gross_amount) : '';
    const sigSource = `${order_id}${status_code}${gross_amount}${serverKey}`;

    const expectedBuf = Buffer.from(crypto.createHash('sha512').update(sigSource).digest('hex'), 'utf8');
    ...
    if (!sigValid) {
      console.warn('paymentNotification: invalid signature_key', { order_id, expected: expectedBuf.toString(), received: receivedBuf.toString() });
