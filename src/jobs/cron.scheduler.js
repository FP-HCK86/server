  ...
  static start() {
    ...
    cron.schedule('*/5 * * * *', async () => {
      try {
        ...
        // Gunakan waktu WIB, lalu konversi ke UTC untuk query
        const nowWIB = moment().tz('Asia/Jakarta');
        const now = nowWIB.utc().toDate(); // UTC untuk query MongoDB
        const fiveMinutesFromNow = nowWIB.add(5, 'minutes').utc().toDate(); // UTC
